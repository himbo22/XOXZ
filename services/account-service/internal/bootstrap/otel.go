...
// Return the shutdown function from the initializer.
func InitOtelCollector(cfg config.TelemetryConfig) (shutdown func(context.Context) error, err error) {
	// 1. Prevent server hangs by limiting initialization to 10 seconds.
	...
	if err != nil {
		// Use a background context because initCtx may already have timed out.
		_ = tracerShutdown(context.Background())
		return nil, fmt.Errorf("failed to init meter: %w", err)
	}

	// 2. Collect shutdown errors with errors.Join.
	shutdown = func(shutdownCtx context.Context) error {
		...
		if len(errs) > 0 {
			return errors.Join(errs...) // Report all collected errors once.
		}
